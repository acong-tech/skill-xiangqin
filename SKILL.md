...
- **数据层**：sqlite @ epsilon (112.124.27.213)
- **查询**：受限 WHERE DSL（字段白名单 + 操作符白名单 + 值白名单）
- **私信**：A 写信 → 进 B 的信箱（永远在，保留 30 天）+ 可选通过 B 的 agent gateway 推通知
- **通知去重**：B 有未读且 < 12h 不再推；≥ 12h 兜底再提醒一次
- **隐私红线**：手机号 HMAC-SHA256；通信双方 agent_gateway_url 不互相透露（xiangqin 代理）
...
Claude: [跑] xq register 13800001111 → request_id=01K...
Claude: 把手机收到的 6 位验证码告诉我
用户：123456
...
机制：每次命令本地读 `~/.xiangqin/version-cache.json`；缓存 >24h 则后台子进程异步查 PyPI 写缓存，**不阻塞当前命令**。失败全静默。
...
xiangqin 客户端（`xq`）**不需要 vault**。服务端（epsilon）通过 vault 声明式管理凭证：

- `vault.json` 声明 aliyun / aliyun_sms / alipay 三把凭证
- `vault install` 产 `.vault/secrets.json`
- 代码 `creds.get("aliyun_sms")` 读

详见 [docs/usage/vault-integration.md](docs/usage/vault-integration.md)。

## 自治单体实验

除 vault 外不依赖生态里其它 CLI。跨项目协作**不走 mailbox**（自治实验期，拒收跨项目邮件）。

## 更多

- [文档站](https://agentaily-xiangqin.pages.dev/) —— 设计 / 装机 / CLI / 运维 / 安全 / FAQ
- [GitHub](https://github.com/yarnovo/xiangqin) —— 源码 / ADR / Issues
